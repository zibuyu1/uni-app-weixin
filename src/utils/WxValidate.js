...
class WxValidate {
  ...
  __initMethods() {
    ...
    that.methods = {
       ...
       */
      emoj(value) {
        return that.optional(value) || !(/\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDE4F]/.test(value))
      },
      /**
       * 验证电子邮箱格式
       */
      email(value) {
        return that.optional(value) || /^([a-zA-Z0-9_\-\.]+)@(([a-zA-Z0-9\-]+\.)+)([a-zA-Z]{2,10}|[0-9]{1,3})$/.test(value)
       ...
       */
      account(value) {
        return that.optional(value) || /^[1][0-9]{10}$/.test(value) || /^([a-zA-Z0-9_\-\.]+)@(([a-zA-Z0-9\-]+\.)+)([a-zA-Z]{2,10}|[0-9]{1,3})$/.test(value)
      },
      /**
       * 验证手机格式
       */
